subroutine goda
  ...
  do while ( abs(sup(n,1)) > cv .and. ((n-1)*dt < Temps))
	...
	& - 0.25*dt*(y(j+1)-y(j-1))*(p1(i+1,j)-p1(i-1,j))
	...
	& - 0.25*dt*(x(i+1)-x(i-1))*(p1(i,j+1)-p1(i,j-1))
      end do
    end do
    ...
    call RESOL_THOMAS(Nx,Ny,Dg3,Lw3,Up3,SMp,phi)

    ! ========================= Etape 3 : résolution directe de u(n+1)  et p(n+1) ==========
    ! ======================================================================================	
    ...
    do i=1,Nx-1
      do j=1,Ny-1
	ux2(i,j) = usx(i,j) - 0.25*(dt*(y(j+1)-y(j-1)))*(phi(i+1,j)-phi(i-1,j))/Vm(i,j)
	uy2(i,j) = usy(i,j) - 0.25*(dt*(x(i+1)-x(i-1)))*(phi(i,j+1)-phi(i,j-1))/Vm(i,j)
      end do
    end do	
...
! 	endif   
	...
	sup(n,2) = max(sup(n,2),abs(p2(i,j)))
! 	sup(n,3) = max(sup(n,3),Psi(i,j))		  

	enddo
      enddo

      call save_VTK(ech,ux2,uy2,p2,x,y,n,Nx,Ny,Psi)	  
  ...
  write(*,*)
  write(*,*) '==================================================================================================='
  write(*,'(a,i3.3,x,a,f16.6,a)') "fin simulation Goda n°",ech,'temps "simulation"', (n-1)*dt,'s'
